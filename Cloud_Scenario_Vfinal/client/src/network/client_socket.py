...
class ClientSocket:
    ...
    # =========================
    # GOODBYE
    # Notify server of a clean, intentional session close so it
    # does not treat the disconnect as an unexpected error.
    # =========================
    def send_goodbye(self, sock):
        header = create_header(
            msg_type=TYPE_GOODBYE,
            token=self.token
        )
        try:
            sock.sendall(serialize_message(header))
        except OSError:
            pass  # socket may already be closing — that's fine
